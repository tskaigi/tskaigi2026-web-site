...
module.exports = async ({ core, context }: Args) => {
  ...
  const imagePath = `/day-staff/${githubUserId}${ext || ".png"}`;
  fs.mkdirSync("public/day-staff", { recursive: true });
  ...
  // ファイルに書き込む
  fs.mkdirSync("src/constants/day-staff", { recursive: true });
  const filePath = `src/constants/day-staff/${githubUserId}.json`;
